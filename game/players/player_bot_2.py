...
class PlayerBot:
            ...
            async with self.session.post(url, json={}) as response:
                if response.status == 200:
                    ...
                    if data.get("success"):
                        ...
                        print(f"[玩家Bot2] 成功加入房间 {room_id}，玩家key: {self.player_key}")
                        return True
                    else:
                        print(f"[玩家Bot2] 加入房间失败: {data.get('message')}")
                        return False
                else:
                    error_text = await response.text()
                    print(f"[玩家Bot2] 加入房间失败，HTTP {response.status}: {error_text}")
                    return False
        except Exception as e:
            print(f"[玩家Bot2] 加入房间异常: {e}")
            return False
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 200:
                    data = await response.json()
                    if data.get("success"):
                        print(f"[玩家Bot2] 确认开始游戏: {data.get('message')}")
                        return True
                    else:
                        print(f"[玩家Bot2] 开始游戏失败: {data.get('message')}")
                        return False
                else:
                    error_text = await response.text()
                    print(f"[玩家Bot2] 开始游戏失败，HTTP {response.status}: {error_text}")
                    return False
        except Exception as e:
            print(f"[玩家Bot2] 开始游戏异常: {e}")
            return False
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    print(f"[玩家Bot2] 获取游戏状态失败，HTTP {response.status}")
                    return None
        except Exception as e:
            print(f"[玩家Bot2] 获取游戏状态异常: {e}")
            return None
            
    async def connect_websocket(self):
        """连接WebSocket接收实时更新"""
        if not self.room_id:
            print("[玩家Bot2] 尚未加入房间，无法连接WebSocket")
            return
            ...
            async with websockets.connect(ws_url) as websocket:
                ...
                async for message in websocket:
                    try:
                        ...
                        print(f"[玩家Bot2] 收到无效JSON消息: {message}")
                        
        except Exception as e:
            print(f"[玩家Bot2] WebSocket连接异常: {e}")
    ...
    async def handle_websocket_message(self, data: dict):
        ...
        if msg_type == "game_state":
            ...
            print(f"[玩家Bot2] 游戏状态更新: {state}")
            
            if state == "playing" and current_player == self.player_key:
                print(f"[玩家Bot2] 轮到我出牌了！")
                await self.play_card(game_data)
                
        elif msg_type == "error":
            print(f"[玩家Bot2] 收到错误消息: {data.get('message')}")
    ...
    async def play_card(self, game_state: dict):
        ...
        if hand_cards:
            # 简单策略：出最后一张牌（与Bot1不同的策略）
            card_to_play = hand_cards[-1]
            print(f"[玩家Bot2] 准备出牌: {card_to_play.get('name')} (ID: {card_to_play.get('id')})")
            
            # TODO: 这里需要调用出牌API，但目前服务器还没有实现
            print(f"[玩家Bot2] 注意：服务器尚未实现出牌API，无法实际出牌")
        else:
            print(f"[玩家Bot2] 手牌为空，无法出牌")
            
    async def run(self, room_id: str):
        """运行机器人"""
        print(f"[玩家Bot2] 启动，准备加入房间: {room_id}")
        ...
        while wait_time < max_wait_time:
            ...
            if game_state and game_state.get("state") == "ready":
                print(f"[玩家Bot2] 房间已准备就绪，开始游戏确认")
                break
            await asyncio.sleep(1)
            wait_time += 1
            
        if wait_time >= max_wait_time:
            print(f"[玩家Bot2] 等待超时，退出")
            return
            
        # 等待额外1秒后确认开始游戏（比Bot1稍快一点）
        await asyncio.sleep(1)
...
async def main():
    if len(sys.argv) != 2:
        print("使用方法: python player_bot_2.py <房间ID>")
        sys.exit(1)
