...
class SimpleLarkAuth:
    ...
    def __init__(self, app_id: str, app_secret: str, redirect_uri: str = "http://localhost:8080/callback",
                 token_file: str = "feishu_token.json"):
        self.APP_ID = app_id
...
class SimpleLark:
    ...
    def _post_req(self, url: str, headers: Dict[str, str], req_body: Dict[str, Any],
                  param: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
    ...
    def post_req(self, url: str, headers: Optional[Dict[str, str]] = None, req_body: Optional[Dict[str, Any]] = None,
                 param: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        if headers is None:
            headers = {}
        if req_body is None:
    ...
    def get_req(self, url: str, headers: Optional[Dict[str, str]] = None, param: Optional[Dict[str, str]] = None) -> \
    Dict[str, Any]:
        if headers is None:
            headers = {}
        if "Authorization" not in headers:
    ...
    def bitable_list(self, app_token: str, table_id: str, filter_dict: Optional[Dict[str, str]] = None,
                     page_token: str = "") -> tuple[List[Dict[str, Any]], Optional[str]]:
        """分页获取多维表格记录"""
    ...
    def get_filtered_records(self, table_name: str = "default", field_name: str = "", field_value: str = "") -> List[
        Dict[str, Any]]:
        """获取筛选后的记录"""
        if field_name and field_value:
            ...
            for record in all_records:
                ...
                if isinstance(field_val, list):
                    if field_val and isinstance(field_val[0], dict) and 'name' in field_val[0]:
                        display_value = ", ".join([item['name'] for item in field_val])
                    else:
                        display_value = str(field_val)
                ...
                # 进行模糊匹配（包含即可）
                if field_value in display_value:
...
if __name__ == "__main__":
    ...
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e0b3a5c362100e")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "L8yFopfRxrFpoEKWl2zF3bsrDCyfcLEB")
