...
# vLLM 引擎配置
llm = vllm.LLM(
    model="/inspire/hdd/project/embodied-multimodality/public/jzzhou/Qwen3-8B",
...
def generate_prompt(query: str) -> str:
    ...
    为单个查询生成prompt
    """
    system_content = "你是一个精通代码修复的 AI 助手，拥有两种工作模式：1. 代理模式（<|AGENT|>）：当用户没有明确指出问题时，你会先思考并使用工具调试代码，分析潜在问题。2. 编辑模式（<|EDIT|>）：当用户明确指出错误（如 SyntaxError、NameError 等），你可以直接修复并返回修改后的代码。\
        你有两个可调用的工具：\
- python：用于执行和调试代码\
- editor：用于比较和编辑原始代码与修改后的代码\
在回答时，请按以下结构返回：\
<think>...你的思考过程...</think>\
<|AGENT|> 或 <|EDIT|>\
{'name': '...', 'arguments': {...}}\
确保每个回答包含一个 `<think>` 思考段落，并标明你使用的模式，再调用相应工具。" 
