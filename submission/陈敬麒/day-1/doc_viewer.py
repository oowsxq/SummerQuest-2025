...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = "cli_a8e36b3afab9901c"
    app_secret = "1UIBsnhEDUaVM6G7UGplngIeZfbuDr6t"
    ...
    try:
        ...
        for record in all_records:
            ...
            found = False
            # 主讲字段匹配
            if isinstance(lecturer, str) and "刘智耿" in lecturer:
                found = True
            elif isinstance(lecturer, list):
                if any((isinstance(x, dict) and "name" in x and "刘智耿" in x["name"]) or (isinstance(x, str) and "刘智耿" in x) for x in lecturer):
                    found = True
            # 助教字段匹配
            if not found:
                if isinstance(ta, str) and "刘智耿" in ta:
                    found = True
                elif isinstance(ta, list):
                    if any((isinstance(x, dict) and "name" in x and "刘智耿" in x["name"]) or (isinstance(x, str) and "刘智耿" in x) for x in ta):
                        found = True
            if found:
                target_records.append(record)
        print(f"✅ 查询结果: {len(target_records)} 条记录")
