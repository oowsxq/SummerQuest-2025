...
def generate_prompt(query: str) -> str:
    """
    为单个查询生成prompt，强制要求输出符合 output_checker.py 检查格式
    """
    # 更严格的 system prompt，包含标准示例
    system_content = (
        "请严格按照如下格式回答：\n"
        "<think>\n你的思考过程\n</think>\n"
        "<|AGENT|>\n"
        '{"name": "python", "arguments": {"code": "这里是要执行的Python代码"}}\n'
        "<|EDIT|>\n"
        '{"name": "editor", "arguments": {"original_code": "原始代码", "modified_code": "修改后的代码"}}\n\n'
        "请确保输出中包含 <think> 部分、<|AGENT|> 和 <|EDIT|>，并且每个标记后都要有对应的 JSON 格式函数调用。"
    )
