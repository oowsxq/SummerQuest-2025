...
class SimpleLark:
    ...
    def get_filtered_records(self, table_name: str = "default", field_name: str = "", field_value: str = "") -> List[Dict[str, Any]]:
        ...
        if not field_name or not field_value:
            print("⚠️ 警告: 未提供 field_name 或 field_value，将返回所有记录。")
            return self.get_records(table_name)

        # 1. 解析 field_name，支持 OR 查询
        fields_to_check = field_name.split('|')
        #print(f"fields_to_check )
        ...
        for record in all_records:
            ...
            # 2. 遍历所有要检查的字段名 (实现 OR 逻辑)
            for name_to_check in fields_to_check:
                if name_to_check in fields:
                    field_val = fields[name_to_check]
                    
                    # 3. 智能匹配逻辑，核心修复点
                    # a) 如果字段值是列表 (处理人员类型)
                    if isinstance(field_val, list):
                        for item in field_val:
                            # 检查列表中的每个元素是否是人员字典并匹配名字
                            if isinstance(item, dict) and item.get("name") == field_value:
                                is_match_found = True
                                break # 找到匹配，跳出内层 item 循环
                    # b) 其他情况，统一转为字符串进行包含匹配
                    elif field_value in str(field_val):
                        is_match_found = True

                # 如果在本记录的某个字段中找到匹配，则无需再检查其他字段
                if is_match_found:
                    break # 跳出外层 name_to_check 循环
            
            # 如果本条记录经验证后确有匹配，加入最终结果
            if is_match_found:
...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e1d79025d1d00b")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "0JkpIi5JeekIxk3ckXJZ9cdvB0ecyzT3")
  ...
  #  print("1. 获取所有记录:")
    try:
        ...
        # 显示前3条记录的关键信息
        # for i, record in enumerate(all_records[:4]):
        #     print(f"\n记录 {i+1}:")
        #     fields = record.get("fields", {})
        #     print(fields)
        #     # 只显示关键字段
  ...
  #  field_name = "主讲"  # 替换为你的字段名
    field_name = "主讲"
    field_name1 = "助教"
    field_name_list = ["助教","主讲"]
    field_value = "刘智耿"
    # 准备筛选条件
  #  field_name_single = "主讲"
   # field_name_or = "主讲|助教"
    # search_value = "郑逸宁"
    ...
    try:
        ...
        for field_name in field_name_list:
            filtered_records = lark.get_filtered_records("default", field_name, field_value=field_value)
