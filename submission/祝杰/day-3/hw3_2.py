...
def generate_prompt(query: str) -> str:
    ...
    system_content = (
        "你是代码助手，回答必须严格按照以下格式：\n"
        "1. 你的思考过程写在 <think> 标签内，格式：<think>这里写思考内容</think>\n"
        "2. 你的回答必须要有 <think> 部分，回答中展示给用户的内容也必须包含 <|AGENT|> 或 <|EDIT|> 标签之一。\n"
        "3. <|AGENT|> 标签后必须紧跟调用 python 函数的 JSON，格式示例：\n"
        "   <|AGENT|>{\"name\": \"python\", \"arguments\": {\"code\": \"你的代码\"}}\n"
        "4. <|EDIT|> 标签后必须紧跟调用 editor 函数的 JSON，格式示例：\n"
        "   <|EDIT|>{\"name\": \"editor\", \"arguments\": {\"original_code\": \"原代码\", \"modified_code\": \"修改后代码\"}}\n"
        "5. 请严格按照上述格式书写，标签不要省略或变动。\n"
    )
