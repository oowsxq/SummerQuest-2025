...
class SimpleLarkAuth:
    ...
    def __init__(self, app_id: str, app_secret: str, redirect_uri: str = "http://localhost:8081/callback", token_file: str = "feishu_token.json"):
        self.APP_ID = app_id
    ...
    def _get_auth_code(self) -> Optional[str]:
        ...
        server = AuthServer(("localhost", 8081), AuthHandler)
...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = "cli_a8e137287f3d900b"
    app_secret = "QExwaaYyTymq5n6LrkSPCeDnqBnn7xnu"
    ...
    # 3.查询 主讲 或 助教 中包含 “刘智耿” 的条目
    print("\n3. 查询主讲或助教中包含“刘智耿”的记录:")
    try:
        all_records = lark.get_records()
        filtered_by_lecturer = []
        for record in all_records:
            fields = record.get("fields", {})
            for key in ["主讲", "助教"]:
                if key in fields:
                    ...
                    if isinstance(val, list):
                        ...
                        if any("刘智耿" in name for name in names):
                            filtered_by_lecturer.append(record)
                            break
                    elif isinstance(val, str) and "刘智耿" in val:
                        filtered_by_lecturer.append(record)
                        break

        print(f"共找到包含“刘智耿”的记录 {len(filtered_by_lecturer)} 条")
        for i, record in enumerate(filtered_by_lecturer):
            print(f"\n匹配记录 {i+1}:")
            fields = record.get("fields", {})
            for field_name_item in ["主讲", "助教", "日期", "课程"]:
                if field_name_item in fields:
