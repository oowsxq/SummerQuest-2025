...
class SimpleLark:
    ...
    def bitable_list(self, app_token: str, table_id: str, filter_dict: Optional[Dict[str, str]] = None, page_token: str = "") -> tuple[List[Dict[str, Any]], Optional[str]]:
        ...
        if rsp_dict.get("code", -1) == 0:
            ...
            print(f"ops 获取记录失败: {rsp_dict.get('msg', '未知错误')}")
            return [], None
    ...
    def get_records(self, table_name: str = "default") -> List[Dict[str, Any]]:
        ...
        print(f"ok 成功获取 {len(all_records)} 条记录")
        return all_records
    ...
    def get_filtered_records(self, table_name: str = "default", field_name: str = "", field_value: str = "") -> List[Dict[str, Any]]:
        ...
        if field_name and field_value:
            ...
            for record in all_records:
                ...
                # print(isinstance(field_val, str),field_val) 
...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e1c8089a26500c")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "mTN2SDkiNiuxWe5vFEu96cYpxHcgnajM")
    ...
    if app_id == "your_app_id" or app_secret == "your_app_secret":
        print("ops 错误：请设置环境变量 FEISHU_APP_ID 和 FEISHU_APP_SECRET")
        print("\n设置方法：")
    ...
    if token_info:
        print(f"  - Access Token有效: {'ok' if token_info['access_token_valid'] else 'ops'}")
        print(f"  - Refresh Token有效: {'ok' if token_info['refresh_token_valid'] else 'ops'}")
    ...
    try:
        ...
        print(f"筛选条件: {field_name1} = {field_value} 或 {field_name2} = {field_value}")
        print(f"ok 筛选结果: {len(filtered_records1)+len(filtered_records2)} 条记录")
        
        tmp=filtered_records1+filtered_records2
