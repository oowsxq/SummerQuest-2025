...
if __name__ == "__main__":
    ...
    app_id = 'cli_a8e0013ae9f8500b'
    app_secret = 'VBV7FexYDBLMbIlePaADnhqbYqdvQQL8'
                    ...
                    if isinstance(val, list):
                        for item in val:
                            if isinstance(item, dict) and item.get("name") == target_name:
                                found = True
                    elif isinstance(val, str) and target_name in val:
                        found = True
            if found:
                matched_records.append(record)

        print(f"✅ 匹配到 {len(matched_records)} 条记录")
