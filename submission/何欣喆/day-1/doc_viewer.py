    ...
    def get_filtered_records(self, table_name: str = "default", field_name: str = "", field_value: str = "") -> List[Dict[str, Any]]:
        ...
        if field_name and field_value:
            ...
            for record in all_records:
                ...
                # print(f"Find record {field_name}: {field_val}")
                # 支持字符串包含匹配
                if isinstance(field_val, str) and field_value in field_val:
                    filtered_records.append(record)
                elif isinstance(field_val, dict) and any(field_value in str(val) for val in field_val.values()):
                    filtered_records.append(record)
                elif isinstance(field_val, list):
                    matched = False
                    for sub_val in field_val:
                        assert isinstance(sub_val, dict), "Not implemented value type"
                        if field_value in sub_val.values():
                            matched = True
                            break
                    if matched:
                        filtered_records.append(record)     
...
if __name__ == "__main__":
    ...
    log_file = open('hw1.log', 'w', encoding='utf-8')
    sys.stdout = log_file
    
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e1d1e9848e900e")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "fo8f65qJfawf3IbKtvem4gMrvYwBhut3")
    ...
    try:
        ...
        for field_name in field_names:
            new_records = lark.get_filtered_records("default", field_name, field_value)
            print(f"筛选条件: {field_name} = {field_value}")
            print(f"✅ 本轮新增匹配: {len(new_records)} 条")
        ...
        print(f"\n🎯 总共筛选出 {len(filtered_records)} 条记录。")
        
        # 打印所有筛选出的记录               
        for i, record in enumerate(filtered_records[:len(filtered_records)]):
            print(f"\n记录 {i+1}:")
            fields = record.get("fields", {})
            # 只显示关键字段
            for field_name in ["主讲", "助教", "日期", "课程"]:
