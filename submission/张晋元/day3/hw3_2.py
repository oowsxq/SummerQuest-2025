...
tokenizer = AutoTokenizer.from_pretrained("/data-mnt/data/downloaded_ckpts/Qwen3-8B", trust_remote_code=True)
...
def generate_prompt(query: str) -> str:
    ...
    """
    system_content = (
        "你是一个智能代码助手。请在你的回答中，合理地使用特殊词符 <|AGENT|>（表示你作为智能体的思考或行动）"
        "和 <|EDIT|>（表示你对代码的编辑或修正）。"
        "请确保输出中包含这两个特殊词符，并用它们包裹相关内容。"
    )
