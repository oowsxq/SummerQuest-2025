...
class SimpleLark:
    ...
    def search_by_teacher(self, table_name: str = "default", teacher_name: str = "") -> List[Dict[str, Any]]:
        if not teacher_name:
            print("请输入要查询的教师姓名")
            return []
        ...
        for record in all_records:
            ...
            lecturer_str = self._extract_text_from_field(lecturer)
            assistant_str = self._extract_text_from_field(assistant)
            
            if teacher_name in lecturer_str or teacher_name in assistant_str:
...
if __name__ == "__main__":
    ...
    print("\n3. 查询主讲或助教中包含'刘智耿'的条目:")
    try:
        liu_records = lark.search_by_teacher("default", "刘智耿")
        print(f"查询结果: 找到 {len(liu_records)} 条包含'刘智耿'的记录")
        
        if liu_records:
            print("\n 详细记录:")
            for i, record in enumerate(liu_records, 1):
                print(f"\n--- 记录 {i} ---")
                fields = record.get("fields", {})
                
                for field_name in ["主讲", "助教", "日期", "课程", "地点", "备注"]:
                    if field_name in fields:
                        field_value = fields[field_name]
                        display_value = lark._extract_text_from_field(field_value)
                        print(f"  {field_name}: {display_value}")
        else:
            print("未找到包含'刘智耿'的记录")
