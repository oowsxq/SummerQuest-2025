        ...
        if field_name and field_value:
            ...
            for record in all_records:
                ...
                if isinstance(field_val, str):
                    ...
                    if any(
                        isinstance(item, dict) and any(field_value in str(v) for v in item.values())
...
if __name__ == "__main__":
    ...
    another_field_name = "助教"  # 替换为另一个字段名
    field_value = "刘智耿"  # 替换为你要筛选的值
    
    try:
        filtered_records = lark.get_filtered_records("default", field_name, field_value)
        filtered_records += lark.get_filtered_records("default", another_field_name, field_value)
        print(f"筛选条件: {field_name} or {another_field_name} = {field_value}")
