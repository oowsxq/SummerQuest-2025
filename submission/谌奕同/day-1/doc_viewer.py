    ...
    def _get_auth_code(self) -> Optional[str]:
        ...
        class AuthHandler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if "code" in params:
                    # 使用类型转换来访问AuthServer的属性
                    cast(AuthServer, self.server).auth_code = params["code"][0]
...
if __name__ == "__main__":
    ...
    print("\n3. 查询主讲或助教中包含\"刘智耿\"的条目:")
    try:
        all_records = lark.get_records("default")
        liu_zhigeng_records = []
        
        for record in all_records:
            fields = record.get("fields", {})
            found_in_lecturer = False
            found_in_assistant = False
            
            # 检查主讲字段
            lecturer_field = fields.get("主讲", "")
            if isinstance(lecturer_field, list) and lecturer_field:
                # 如果是对象列表，检查name字段
                if isinstance(lecturer_field[0], dict):
                    lecturer_names = [item.get('name', '') for item in lecturer_field]
                    found_in_lecturer = any("刘智耿" in name for name in lecturer_names)
                else:
                    # 如果是字符串列表
                    found_in_lecturer = any("刘智耿" in str(item) for item in lecturer_field)
            elif isinstance(lecturer_field, str):
                found_in_lecturer = "刘智耿" in lecturer_field
            
            # 检查助教字段
            assistant_field = fields.get("助教", "")
            if isinstance(assistant_field, list) and assistant_field:
                # 如果是对象列表，检查name字段
                if isinstance(assistant_field[0], dict):
                    assistant_names = [item.get('name', '') for item in assistant_field]
                    found_in_assistant = any("刘智耿" in name for name in assistant_names)
                else:
                    # 如果是字符串列表
                    found_in_assistant = any("刘智耿" in str(item) for item in assistant_field)
            elif isinstance(assistant_field, str):
                found_in_assistant = "刘智耿" in assistant_field
            
            # 如果在主讲或助教中找到"刘智耿"，添加到结果中
            if found_in_lecturer or found_in_assistant:
                liu_zhigeng_records.append(record)
        
        print(f"✅ 找到 {len(liu_zhigeng_records)} 条包含\"刘智耿\"的记录:")
        
        if len(liu_zhigeng_records) == 0:
            print("  📝 未找到相关记录")
        else:
            for i, record in enumerate(liu_zhigeng_records):
                print(f"\n记录 {i+1}:")
                fields = record.get("fields", {})
                
                # 显示所有关键字段
                for field_name_item in ["主讲", "助教", "日期", "课程", "内容"]:
                    if field_name_item in fields:
                        field_value_item = fields[field_name_item]
                        # 简化显示复杂字段
                        if isinstance(field_value_item, list) and field_value_item:
                        ...
                        # 高亮显示包含"刘智耿"的字段
                        if field_name_item in ["主讲", "助教"] and "刘智耿" in display_value:
                            print(f"  {field_name_item}: {display_value} ⭐")
                        else:
                            print(f"  {field_name_item}: {display_value}")
                
    except Exception as e:
        print(f"❌ 查询刘智耿相关记录失败: {str(e)}")
