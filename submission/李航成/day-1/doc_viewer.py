...
if __name__ == "__main__":
    ...
    app_id = 'cli_a8e167a91578900c'
    app_secret = 'lDSaC8rhqgE1uasR7Pk86fdicKXr2bKI'
    ...
    try:
        ...
        for record in all_records:
            ...
            # 检查主讲字段
            if "主讲" in fields:
                lecturer_field = fields["主讲"]
                if isinstance(lecturer_field, list):
                    for item in lecturer_field:
                        if isinstance(item, dict) and 'name' in item:
                            if target_person in item['name']:
                                found = True
                                break
                        elif isinstance(item, str) and target_person in item:
                            found = True
                            break
                elif isinstance(lecturer_field, str) and target_person in lecturer_field:
                    found = True
            
            # 检查助教字段
            if not found and "助教" in fields:
                assistant_field = fields["助教"]
                if isinstance(assistant_field, list):
                    for item in assistant_field:
                        if isinstance(item, dict) and 'name' in item:
                            if target_person in item['name']:
                                found = True
                                break
                            ...
                            found = True
                            break
                elif isinstance(assistant_field, str) and target_person in assistant_field:
                    found = True
            
            if found:
                filtered_records.append(record)
        
        print(f"筛选条件: 主讲或助教中包含 '{target_person}'")
