...
'''
从 handout/day-3 中 复制 所需的文件到自己的目录下运行，完成作业后仅在 submission 目录下提交文件。
1. 使用 Qwen3-8B 做为基座模型，为其添加特殊词符(Special Token)  <|AGENT|>  和 <|EDIT|>。然后用 Tokenizer 解码文件 query_and_output.json 中的内容，将答案保存在 hw3_1.json 中。（不用加载模型，只需使用 Tokenizer）hw3_1.json 中需要包含加入 special token 后的 Token IDs。
2. 设计不同的系统提示词（System Prompt），让模型在不训练的情况下，尽可能的生成包含这些特殊词符的输出结果，将答案保存在 hw3_2.json 中。生成答案后可以使用 output_checker.py 检查答案，因为模型没有经过训练，完全使用 prompt 的方式未必能使模型完全按照自己预期输出，大家尽力完成即可
'''
...
for item in tasks:
    ...
    # 编码并获取 token IDs
    ids = tokenizer([merged_text])["input_ids"][0]
    # 解码验证
