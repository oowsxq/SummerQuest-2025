...
if __name__ == "__main__":
    ...
    try:
        log_file = open("hw1.log", "w", encoding="utf-8", buffering=1)
        sys.stdout = log_file
        sys.stderr = log_file
    except Exception as e:
        print(f"日志文件写入失败: {e}")
        exit(1)
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e1d3d32b3c100b")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "desc8g6sPmDR7I7ANAFaldasmkGdXg87")
    ...
    try:
        ...
        print(f"找到 {len(matching_records)} 条包含'{target_name}'的记录:")
        print()
        ...
        for i, record in enumerate(matching_records, 1):
            ...
            for field_name in ["课程", "主讲", "助教", "日期", "地点"]:
