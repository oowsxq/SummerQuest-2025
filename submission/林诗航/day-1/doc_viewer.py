...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e1cd8fdabb100b")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "amcxZPvoJjkBerpfVHxG8bL8v1MLBi64")
    ...
    # 2. 筛选特定字段的记录
    print("\n2. 筛选记录演示:")
    def my_filter(field_val, target):
    ...
    try:
        ...
        for record in all_records:
            fields = record.get("fields", {})
            field_val1 = fields.get("主讲", "")
            field_val2 = fields.get("助教", "")

            if my_filter(field_val1, target) or my_filter(field_val2, target):
                filtered_records.append(record)
