...
# 将标准输出重定向到文件
sys.stdout = open('hw1.log', 'w', encoding='utf-8')
...
class SimpleLarkAuth:
    ...
    def _get_auth_code(self) -> Optional[str]:
        ...
        auth_url = (
            ...
            f"&scope=bitable:app:readonly%20offline_access"
        )
...
class SimpleLark:
    ...
    def get_filtered_records(self, table_name: str = "default", field_name_str: str = "", field_value: str = "") -> List[Dict[str, Any]]:
        """获取筛选后的记录
        ...
        for field_name in field_name_list:
            if field_name and field_value:
                ...
                for record in all_records:
                    ...
                    name_list = []
                    if len(field_val) != 0 and isinstance(field_val, list):
                        for field_ori in field_val:
                            name_list.append(field_ori.get("name", "费森俞"))
                    
                    # 支持字符串包含匹配
                    if field_value in name_list:
...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = "cli_a8e1d35fc6bf500c" #os.environ.get("FEISHU_APP_ID", "cli_a8e1d35fc6bf500c")
    app_secret = "D4O9s43918EVdlCfZ71IyT2B4JWoTpLi" # os.environ.get("FEISHU_APP_SECRET", "36p1h7fKoxcNgXbjIh7yqhliV4eOu0h2")
    ...
    print("=" * 50)
    print("作业1: 创建飞书APP，并利用接口实现文档自动化查询")
    print("=" * 50)
