        ...
        if field_name and field_value:
            ...
            for record in all_records:
                ...
                if isinstance(field_val, str) and field_value in field_val:
                    ...
                    # 如果是列表，检查是否有包含目标值的字典
                    if any(str(item.get('name', '')) == field_value for item in field_val if isinstance(item, dict)):
                        filtered_records.append(record)
...
if __name__ == "__main__":
    ...
    try:
        ...
        matched_records = [record for record in all_records if TARGET_NAME in record.get("fields", {}).get("主讲", "")]
        print(f"✅ 共找到 {len(all_records)} 条记录，其中包含目标姓名 '{TARGET_NAME}' 的记录有 {len(matched_records)} 条")
    ...
    # 2. 筛选特定字段的记录
    print("\n2. 筛选记录演示:")
    field_names = TARGET_PROPERTIES
    field_value = TARGET_NAME
    
    try:
        filtered_records = []
        for field_name in field_names:
            filtered_records.extend(lark.get_filtered_records("default", field_name, field_value))
        filtered_records = list({record['record_id']: record for record in filtered_records}.values())  # 去重
        print(f"筛选条件: {field_names} = {field_value}")
