...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e0355602fa100c")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "oOCbNEUfIlA472kKClzLyplyG6tmmTmK")
    ...
    # 3. 使用 get_records_by_instructor 查询“刘智耿”
    print("\n3. 使用 get_records_by_instructor 查询讲师/助教包含 '刘智耿' 的记录:")
    instructor_name = "刘智耿"
    
    try:
        filtered_records = lark.get_records_by_instructor("default", instructor_name)
        print(f"查询讲师或助教包含: {instructor_name}")
        print(f"✅ 匹配记录数: {len(filtered_records)} 条")
