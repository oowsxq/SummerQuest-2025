...
if __name__ == "__main__":
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e1d44dda39d00c")  # 你的 App ID
    app_secret = os.environ.get("FEISHU_APP_SECRET", "v1IcAq5A28kSeKahUgGb1dImLpOD3BiV") # 你的 App Secret
    bitable_url = "https://fudan-nlp.feishu.cn/base/KH8obWHvqam2Y4sXGGuct2HFnEb?table=tbljlS1fS0UepxBn&view=vewCig26Kk"

    if "你的" in app_id or "你的" in app_secret:
        print("❌ 错误：请在代码中填写你的真实 app_id 和 app_secret")
        exit(1)
    ...
    try:
        ...
        print(f"❌ 初始化或获取数据时发生错误: {e}")
        exit(1)

    output_filename = "hw1.log"
    ...
    with open(output_filename, 'w', encoding='utf-8') as f:
        ...
        for record in all_records:
            ...
            lecturer_list = fields.get("主讲", [])
            ta_list = fields.get("助教", [])
            
            lecturer_names = ""
            if isinstance(lecturer_list, list):
                lecturer_names = ",".join([person.get("name", "") for person in lecturer_list])

            ta_names = ""
            if isinstance(ta_list, list):
                ta_names = ",".join([person.get("name", "") for person in ta_list])
    ...
    if found_count > 0:
        print(f"✅ 查询完成！结果已写入到 {output_filename} 文件中，共找到 {found_count} 条记录。")
    else:
        print(f"ℹ️ 查询完成，未找到符合条件的记录。已生成空的 {output_filename} 文件。")
